...
The compact LGS webview is a tool-first command surface. Conversation receives the available space; persistent branding and explanatory empty-state content are intentionally minimal. The composer exposes added context, honest model-aware context utilization, active provider/model, capability-gated reasoning, execution mode, Send, and Stop. Narrow widths collapse secondary controls without hiding the request input or execution controls.

## Task workflow and observability

Every user submission creates a distinct `RequestExecution` beneath its conversation. One wall-clock timer spans the whole request. Stable phases carry a concise goal, profile assignment, timestamps, status, and optional result. Observable work is appended as structured `ActivityEvent` records under `.lgs/tasks/<request-id>/activity.jsonl`; phase details summarize the work structure, while **Open activity log** opens a live IDE-style trace beside chat. File events can open repository files, command details stay collapsed, and neither surface exposes private chain-of-thought.

Modes are composable policies: **Normal** permits standard engineering execution, **Plan** is inspection/planning without mutation, **Web** is external investigation without repository mutation, and **Research** combines planning, web evidence, implementation, verification, and iteration toward a defined outcome. Stop cancels the active model/tool signal. Completion evidence remains available from the compact verification row and durable task artifacts rather than occupying a parallel dashboard by default.
...
The Settings experience uses visual Appearance selectors, provider controls, Skills, Plugins, Agent Profiles, permissions, usage, verification, and diagnostics. Repository agent configuration remains human-readable under `AGENTS.md` and `.agents/`; the UI can initialize missing structure and create valid skills without overwriting existing files. Appearance cards apply immediately: **Follow VS Code** maps semantic roles to native `--vscode-*` variables, while **Research Paper** and **Warm Dark** provide restrained warm palettes.
