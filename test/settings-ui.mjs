...
const baseState = {
  ...
  skills: [{ id: 'lgs-frontend', name: 'lgs-frontend', description: 'Visible LGS interface work', applicableTasks: ['frontend'], activationRules: [], estimatedTokenCost: 900, scope: 'project', enabled: true, source: 'local', compatibility: ['codex'], routing: { profiles: ['frontend'], activation: 'automatic' }, path: '.agents/skills/lgs-frontend/SKILL.md', supportingFiles: ['visual-language.md'], valid: true, validationErrors: [] }], plugins: [],
  agentProfiles: [{ id: 'frontend', name: 'Frontend', description: 'Visible interface implementation', origin: 'built-in', modified: true, instructions: 'Inspect the rendered interface.', preferredSkills: ['lgs-frontend'], autoLoadedSkills: ['lgs-frontend'], toolPreferences: ['inspect', 'edit', 'commands', 'web', 'verify'], researchBehavior: 'prefer-current-sources', executionBehavior: 'balanced', verificationExpectations: ['Verify rendered behavior.'], policy: { preferences: ['Prefer focused edits.'], defaults: { skills: ['lgs-frontend'], research: 'prefer-current-sources', tools: ['inspect', 'edit', 'verify'] }, requirements: ['Verify rendered behavior.'], restrictions: {} } }],
  settings: [
...
await page.locator('#create-skill').click(); await page.locator('[name="skillName"]').fill('Focused Testing'); await page.locator('[name="work"]').fill('Improve verification work'); await page.locator('[name="desiredExamples"]').fill('Create a focused reproduction first.'); await page.locator('[name="avoid"]').fill('Unrelated rewrites'); await page.locator('[name="priorities"]').fill('Fast feedback; reliable evidence'); await page.locator('[name="constraints"]').fill('Use repository test commands'); await page.locator('#review-skill').click();
const prepareSkill = (await page.evaluate(() => globalThis.__messages)).at(-1); assert.equal(prepareSkill.type, 'prepareSkill'); assert.equal(prepareSkill.activation, 'automatic');
await dispatch({ type: 'skillGeneration', draft: { id: prepareSkill.sessionId, skillName: 'Focused Testing', answers: { work: 'Improve verification work', desiredExamples: 'Create a focused reproduction first.', avoid: 'Unrelated rewrites', priorities: 'Fast feedback; reliable evidence', constraints: 'Use repository test commands', scope: 'project', profiles: [], activation: 'automatic' }, patterns: ['Prioritize fast feedback', 'Avoid unrelated rewrites'], proposal: { name: 'Focused Testing', description: 'Improve verification work', instructions: 'Create a focused reproduction first.', scope: 'project', routing: { profiles: [], activation: 'automatic' } }, status: 'review' }, diff: '--- current/SKILL.md\n+++ proposed/SKILL.md\n+Create a focused reproduction first.' });
assert.match(await page.locator('.proposal').innerText(), /Inferred pattern/); assert.equal(await page.locator('#approve-skill').isVisible(), true); await page.screenshot({ path: '/tmp/lgs-agent-skill-generation.png', fullPage: true }); await page.locator('#close-skill').click();
await page.locator('[data-nav="agents"]').click(); assert.match(await page.locator('.profile-list').innerText(), /Default profile · Modified/); await page.locator('[data-edit-profile="frontend"]').click(); assert.match(await page.locator('#profile-form').innerText(), /Hard restrictions/); await page.screenshot({ path: '/tmp/lgs-agent-profile-editor.png', fullPage: true }); await page.locator('#close-profile').click();
